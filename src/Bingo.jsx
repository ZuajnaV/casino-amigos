...
function Prize(gross) { return Math.round(gross * (1 - RAKE_PCT)); }

// Color de columna BINGO
function colColor(n) {
  const idx = Math.min(4, Math.floor((n - 1) / 15));
  return ["#ef4444","#3b82f6","#22c55e","#f59e0b","#a855f7"][idx];
}

// ── CAMBIO 1: letra de columna ────────────────────────────────
function colLetter(n) {
  return BINGO_LETTERS[Math.min(4, Math.floor((n - 1) / 15))];
}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {myCount > 0 && (
        <button onClick={onReady} disabled={isReady || loading} style={{
          ...
          border: "none", borderRadius: 10, padding: "13px", fontSize: 15, fontWeight: 900,
          cursor: isReady ? "not-allowed" : "pointer", color: isReady ? "#444" : "#000",
        }}>
...
export default function BingoGame({ profile, balance, setBalance, onBack }) {
  ...
  // CAMBIO 1: última bola que persiste después de que justCalled se limpia
  ...
  // ── Countdown reset por cada bola nueva ──────────────────────────────────
  useEffect(() => {
    ...
    countdownRef.current = setInterval(() => {
      setCountdown(prev => prev <= 1 ? secs : prev - 1);
    }, 1000);
    return () => { if (countdownRef.current) clearInterval(countdownRef.current); };
  }, [calledBalls.length, phase]);

  // ── CAMBIO 3: guardar playerMarked en DB con debounce ───────────────────
  // Necesita: ALTER TABLE bingo_players ADD COLUMN IF NOT EXISTS marked_numbers INT[] DEFAULT '{}';
  useEffect(() => {
    if (phase !== "playing" || !room?.id) return;
    if (saveMarkedRef.current) clearTimeout(saveMarkedRef.current);
    saveMarkedRef.current = setTimeout(async () => {
      await supabase.from("bingo_players")
        .update({ marked_numbers: [...playerMarked] })
        .eq("room_id", room.id)
        .eq("user_id", profile.id);
    }, 400);
  }, [playerMarked]); // eslint-disable-line
  ...
  const subscribeToRoom = useCallback((roomId) => {
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "bingo_rooms", filter: `id=eq.${roomId}` },
        ({ new: newRoom }) => {
          if (!newRoom) return;
          ...
          if (balls.length > (roomRef.current?.called_balls?.length || 0)) {
            ...
            setJustCalled(lb);
            setLastBall(lb);   // CAMBIO 1: persiste
  ...
  async function loadRooms() {
    const { data } = await supabase.from("bingo_rooms").select("*, bingo_players(count)")
      .eq("status", "waiting").order("created_at", { ascending: false }).limit(10);
    setRooms(data || []);
  }
  ...
  async function loadRoom(roomId) {
    const [{ data: roomData }, { data: players }] = await Promise.all([
      ...
      supabase.from("bingo_players").select("user_id, username, avatar, card_count, ready, cards, marked_numbers").eq("room_id", roomId),
    ]);
  ...
  useEffect(() => {
    loadRooms();
    const ch = supabase.channel("bingo_rooms_list")
  ...
  async function createRoom() {
    ...
    await supabase.from("bingo_players").insert({
      ...
      avatar: profile.avatar || "🎲", card_count: 0, ready: false, cards: [], marked_numbers: [],
    });
  ...
  async function joinRoom(roomId) {
    ...
    if (!existing) {
      const { count: pc } = await supabase.from("bingo_players").select("id", { count: "exact" }).eq("room_id", roomId);
      if (pc >= MAX_PLAYERS) { alert("Sala llena"); setLoading(false); return; }
      await supabase.from("bingo_players").insert({
  ...
  async function buyCard() {
    ...
    setBalance(newBalance); balRef.current = newBalance;
    const roomId = room?.id;
    await supabase.from("bingo_players").update({ card_count: updatedCards.length, cards: updatedCards }).eq("room_id", roomId).eq("user_id", profile.id);
    const { data: ap } = await supabase.from("bingo_players").select("card_count").eq("room_id", roomId);
    const tc = (ap || []).reduce((s, p) => s + (p.card_count || 0), 0);
    await supabase.from("bingo_rooms").update({ prize: Prize(tc * CARD_PRICE) }).eq("id", roomId);
  ...
  async function markReady() {
    ...
    if (players?.every(p => p.ready && p.card_count > 0) && players.length >= 2) await startGame(roomId);
    setLoading(false);
  }

  async function startGame(roomId) {
    const balls = Array.from({length: 75}, (_, i) => i + 1).sort(() => Math.random() - 0.5);
    await supabase.from("bingo_rooms").update({ status: "playing", all_balls: balls, called_balls: [] }).eq("id", roomId);
    startBallExtraction(roomId, balls);
  }

  // ── CAMBIO 3: detección de victoria basada en marked_numbers ──────────────
  ...
  function startBallExtraction(roomId, balls) {
    ...
    const interval = setInterval(async () => {
      ...
      // Leer marked_numbers de cada jugador (lo que marcaron manualmente)
      const { data: players } = await supabase.from("bingo_players")
        .select("user_id, cards, marked_numbers").eq("room_id", roomId);
      const { data: roomData } = await supabase.from("bingo_rooms").select("prize").eq("id", roomId).single();

      for (const player of (players || [])) {
        // La victoria se basa en lo que EL JUGADOR marcó, no en todas las bolas
        const markedSet = new Set(player.marked_numbers || []);
        ...
        for (const card of (player.cards || [])) {
          ...
          if (winPattern) {
            ...
            const { data: wp } = await supabase.from("profiles").select("balance").eq("id", player.user_id).single();
            if (wp) await supabase.from("profiles").update({ balance: wp.balance + roomData.prize }).eq("id", player.user_id);
            return;
          }
        }
      }
  ...
  async function leaveRoom() {
    ...
    setCalledBalls([]); setLastBall(null); setCountdown(BALL_INTERVAL / 1000);
    setPhase("lobby_list");
    await loadRooms();
  }
  ...
  // ═══════════════════════════════════════════════════════════
  if (phase === "playing") {
    const timerColor = countdown <= 1 ? "#00d4aa" : countdown <= 2 ? "#fbbf24" : "#555";
    ...
    return (
      <div style={{ minHeight:"100vh", background:"#07070f", color:"#fff", fontFamily:"'Georgia',serif", paddingBottom:40 }}>
        ...
        <div style={{ padding:"12px", maxWidth:"100%", margin:"0 auto" }}>
          ...
          {/* ── CAMBIO 1: Bola + Letra + Temporizador ── */}
            ...
            <div style={{ textAlign:"center" }}>
              {lastBall ? (
                <div style={{
                  ...
                  background: justCalled
                    ? `radial-gradient(circle, ${colColor(lastBall)}, ${colColor(lastBall)}aa)`
                  ...
                  transition:"all 0.4s",
                  animation: justCalled ? "ballPop 0.3s ease" : "none",
                }}>{lastBall}</div>
              ) : (
  ...
  if (phase === "finished") {
    ...
    return (
      <div style={{ minHeight:"100vh", background:"#07070f", color:"#fff", fontFamily:"'Georgia',serif", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
        <style>{globalCss}</style>
        <div style={{
          ...
          <div style={{ fontSize:72, marginBottom:8 }}>{iWon ? "🏆" : "🎱"}</div>
          <div style={{
          ...
          <div style={{ display:"flex", gap:12, justifyContent:"center" }}>
            <button onClick={() => {
              ...
              setCalledBalls([]); setLastBall(null); setWinner(null); setMyWin(null);
              setCountdown(BALL_INTERVAL / 1000); loadRooms();
            ...
            <button onClick={onBack} style={{ background:"transparent", border:"1px solid #444", borderRadius:10, padding:"12px 20px", fontSize:14, color:"#888", cursor:"pointer" }}>← Lobby</button>
          </div>
        </div>
      </div>
    );
